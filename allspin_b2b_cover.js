...
const {encoder, decoder, Page, Field, Mino} = require('tetris-fumen');
const fs = require('fs');
...
function placement_path_center(final, field) {
    // bfs I think
    ...
    while (queue.length > 0) {
        ...
        if (field.canFill(operation)) {
            ...
            if (DROP != DROPS.GRAVITY || field.canLock(operation)) {
                // if 20g, dont allow inputs in midair
                

                d_1_steps.push(spin_cw(operation, field));
                d_1_steps.push(spin_ccw(operation, field));
...
function is_placeable(op, field) {
    inputs = ["softDrop", "rotateCW", "rotateCCW", "moveRight", "moveLeft", "DAS_right", "DAS_left"]; // in the fixed arbitrary order we add them here
    ...
    let temp = placement_path_center(op, field);
...
function get_score(
    ...
    for (let page of solution_pages) {
        ...
        if (piece == op.type) {
            ...
            // console.log(is_placeable(op, base_field));
            if (base_field.canLock(op) && is_placeable(op, base_field)) {
                ...
                */
                ...
                if ((activate_b2b || pc) && base_b2b) {b2b = true;}
...
function pick_better_score(so0, so1) {
    if (!so0) {return so1;}
    if (!so1) {return so0;}
    if (so1.b2b_end && !so0.b2b_end) {return so1;} // prioritize b2b
...
function calculate_allspin_b2b_cover(
queues,
data_nohold,
score_cover_filename = undefined, // .csv file to output score cover to
    ...
    for (let queue of nohold_queues) {
        ...
        for (let j = 0; j < data_nohold[queue].length; j++) {
            if (data_nohold[queue][j] == 'O') {
                ...
                let score_obj = get_score(queue, pages, true, 0, 1000000, cumulative_rowsCleared);
                all_sol_scores[j] =  score_obj.b2b_end ? 'O' : 'X'; //score_obj.score;
    ...
    for (let queue of queues) {
        ...
        if (max_score_obj) {
            ...
            let score_cover_row = Array(solutions.length).fill('X');
            for (let hold_queue of hold_reorderings) {
                for (let i = 0; i < solutions.length; i++) {
                    if (!nohold_queues_set.has(hold_queue)) {continue;}
                    score_cover_row[i] = score_cover_row[i] === 'O' || solution_scores_by_nohold_queue[hold_queue][i] === 'O' ? 'O' : 'X';
                }
            }
    ...
    // }
    console.log(`allspin b2b cover: ${num_b2b_queues}/${queues.length} = ${(num_b2b_queues/queues.length*100).toFixed(4)}%`);
...
// Calculate scores using calculate_allspin_b2b_cover() with the loaded CSV file and command-line arguments
const results = calculate_allspin_b2b_cover(
  queues,
  csvFile,
  'output/allspin_b2b_cover.csv',
);

// console.log(results.num_b2b_queues / queues.length);
