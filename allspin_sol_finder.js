...
let score_table_spin_mini = [100, 200, 400, 800];
...
function is_placeable(op, field) {
    // bfs I think
...
function t_spin_checker(op, field) {
    // returns -1 if not t spin; otherwise, returns the kick index (0-4) of the last spin used
...
function get_score(queue, solution_pages, base_b2b = true, cumulative_rowsCleared = undefined, base_field = undefined, base_viz = undefined, base_rowsCleared = undefined) {
    ...
    for (let page of solution_pages) {
        ...
        if (piece == op.type) {
            ...
            if (base_field.canLock(op) && is_placeable(op, base_field)) {
                ...
                if (tspin && GAME === GAMES.JSTRIS && lines_cleared >= 2) {
                    mini = false;
                }

                let activate_b2b = (tspin && lines_cleared > 0) || lines_cleared >= 4;
                if (!activate_b2b && lines_cleared > 0 && !tspin) {
                    // allspin check
                    let immobile = true;

                    let moved_operation_right = op.copy();
                    moved_operation_right.x++;
                    if (base_field.canFill(moved_operation_right)) immobile = false;

                    let moved_operation_left = op.copy();
                    moved_operation_left.x--;
                    if (base_field.canFill(moved_operation_left)) immobile = false;

                    let moved_operation_up = op.copy();
                    moved_operation_up.y++;
                    if (base_field.canFill(moved_operation_up)) immobile = false;
                ...
                if (queue.length <= 1) {
                    ...
                    results.push({ score: score, extra: noteworthy ? [score_event] : [], pcs: +pc, pc_end: pc, b2b_end: b2b, fumen: encoder.encode(viz) });
                    // end of queue is base case for recursive function
                } else {
                    let score_obj = get_score(queue.substring(1), solution_pages, b2b, cumulative_rowsCleared, field, viz, rowsCleared);
                    // otherwise, recursively call score function to get max score on the rest of the queue
                    score_obj.score += score;
                    if (noteworthy) {
                        score_obj.extra.push(score_event);
                    }
                    score_obj.pcs += pc;
    ...
    if (results.length == 0) {
        // no piece placement applied to this piece, this path is a failure
...
function pick_better_score(so0, so1) {
    ...
    if (so1.score > so0.score) {
        return so1;
    }
    if (so0.score > so1.score) {
        return so0;
    }
    if (so0.pc_end && !so1.pc_end) {
        return so0;
    }
    if (so1.pc_end && !so0.pc_end) {
        return so1;
    }
    if (so0.b2b_end && !so1.b2b_end) {
        return so0;
    }
    if (so1.b2b_end && !so0.b2b_end) {
        return so1;
    }
    if (so0.extra.filter((event) => event.tspin).length < so1.extra.filter((event) => event.tspin).length) {
        return so0;
    }
    return so1;
}
...
function calculate_all_scores(
    ...
    for (let queue of nohold_queues) {
        ...
        for (let j = 0; j < data_nohold[queue].length; j++) {
            if (data_nohold[queue][j] == "O") {
                ...
                let score_obj = get_score(queue, pages, base_b2b, cumulative_rowsCleared);
    ...
    for (let queue of queues) {
        ...
        if (max_score_obj) {
            ...
            if (max_score_obj.pc_end) {
                num_pc_queues++;
            }
            if (max_score_obj.b2b_end) {
                num_b2b_queues++;
                console.log(max_score_obj.fumen);
            }
            ...
            for (let hold_queue of hold_reorderings) {
                for (let i = 0; i < solutions.length; i++) {
                    if (!nohold_queues_set.has(hold_queue)) {
                        continue;
                    }
    ...
    if (score_cover_filename) {
        writeCSV(score_cover_filename, score_cover_data);
    }
    return {
...
function generatePermutations(bag, permutationLength) {
    ...
    for (let i = 0; i < permutationLength; i++) {
        permutations = permutations.flatMap((p) => elements.filter((e) => !p.includes(e)).map((e) => [...p, e]));
    }
...
function sfinderAllPermutations(inputStr) {
    ...
    for (let inputStr of inputs) {
        inputStr = inputStr.replace("^", "[^]").replace("!", "p" + countPieces(inputStr));
...
if (queues.length > 1) {
    console.log("bro this command is specifically for 1 queue");
    return;
}
