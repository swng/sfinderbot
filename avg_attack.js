...
args.forEach((arg, index) => {
  if (arg.startsWith('initialB2B=')) {
    initialB2B = parseInt(arg.slice(11));
  } else if (arg.startsWith('initialCombo=')) {
    initialCombo = parseInt(arg.slice(13));
    ...
    tempGame = arg.slice(5);
  }
});

const GAMES = {JSTRIS: {}, TETRIO: {}, GUIDELINE: {}};
let GAME =  GAMES.TETRIO; 
// ONLY TETRIO SUPPORTED FOR NOW
// ill make jstris if there's enough demand

let b2b_cutoff_table = [2, 4, 9, 25, 68, 186, 505, 1371, 10000]; // if you put in a b2b higher than 10000 you DESERVE an error
...
let score_table_normal    = [	
								(combo, b2bLevel) => 0,
								(combo, b2bLevel) => tetrioatk.calAtk("SINGLE", b2bLevel, combo),
								(combo, b2bLevel) => tetrioatk.calAtk("DOUBLE", b2bLevel, combo),
								(combo, b2bLevel) => tetrioatk.calAtk("TRIPLE", b2bLevel, combo),
								(combo, b2bLevel) => tetrioatk.calAtk("QUAD", b2bLevel, combo)
							];

let score_table_spin      = [ 
								(combo, b2bLevel) => tetrioatk.calAtk("TSPIN", b2bLevel, combo),	
								(combo, b2bLevel) => tetrioatk.calAtk("TSPIN_SINGLE", b2bLevel, combo),
								(combo, b2bLevel) => tetrioatk.calAtk("TSPIN_DOUBLE", b2bLevel, combo),
								(combo, b2bLevel) => tetrioatk.calAtk("TSPIN_TRIPLE", b2bLevel, combo)
							];
								
let score_table_spin_mini = [	
								(combo, b2bLevel) => tetrioatk.calAtk("TSPIN_MINI", b2bLevel, combo),	
								(combo, b2bLevel) => tetrioatk.calAtk("TSPIN_MINI_SINGLE", b2bLevel, combo),
								(combo, b2bLevel) => tetrioatk.calAtk("TSPIN_MINI_DOUBLE", b2bLevel, combo)
							];
...
function get_180_kicks(operation, initial_rotation) {
	if (GAME === GAMES.GUIDELINE) {return []}
	if (operation.type == 'I') {
...
function t_spin_checker(op, field) { // returns -1 if not t spin; otherwise, returns the kick index (0-4) of the last spin used
	...
	for (let kick of ccw_kicks) {
      ...
      // return encoder.encode( [ {field: field, operation: kick} ] )
			let temp_kicks = get_cw_kicks(temp, kick.rotation);
      test = [];
      let fail = false
			for (let i = 0; i < 5; i++) {
				temp_kick = temp_kicks[i];
        test.push({i, kick, temp_kick});
				if (field.canFill(temp_kick)) {
					if (temp_kick.x == op.x && temp_kick.y == op.y & !fail) return i /*throw test*/;
					fail = true;
				}
			}
			//return -1; // only first working kick
...
function get_score(
	...
	for (let page of solution_pages) {
		...
		if (piece == op.type) {
			...
			if (base_field.canLock(op) && is_placeable(op, base_field)) {
				...
				if (tspin) {
					if (mini) {
						score += score_table_spin_mini[lines_cleared](combo, b2b);
					} else {
						score += score_table_spin[lines_cleared](combo, b2b);
					}
				} else {
					score += score_table_normal[lines_cleared](combo, b2b);
				}
				...
				/*
				any full t-spin:         yes
				any mini t-spin w/o b2b: no (EPIC NOTE: COMMENTED OUT)
				...
				*/
				...
				if (pc) {
					...
					score += 10; // tetrio only

					// return score;
				}

				if (activate_b2b) {b2b++;}
				else if (lines_cleared > 0) {b2b = 0;}
	...
	if (results.length == 0) { // no piece placement applied to this piece, this path is a failure
		return {score: 0, extra: [], pcs: +false, pc_end: false, b2b_end: false}
		// return -3000; // may want to just return -3000 if working with non *p7 solution queues with dupes
    ...
    // EPIC NOTE: changed to 0 because it fucks with cover score a lot
	}
...
queues,
data_nohold,
base_b2b = 1, // should be true unless it's a skim setup ** i have no idea how to change this so im making it 1?
base_combo = 1, // should be 1 unless TSD in DPC setup was done early
// b2b_end_bonus = 0, // ~0 for 1st, 300 for SDPC, 500 for SDPC w/ TD fallback, 800 for TD ** i dont care lol
	...
	for (let queue of nohold_queues) {
		...
		for (let j = 0; j < data_nohold[queue].length; j++) {
			if (data_nohold[queue][j] == 'O') {
				...
				// compute it
				// queue, solution pages, initial b2b, initial combo
				let score_obj = get_score(queue, pages, base_b2b, base_combo, cumulative_rowsCleared);
...
function generatePermutations(bag, permutationLength) {
  ...
  if (elements.length && elements[0] === '^') {
    ...
    elements = "IOSZJLT".split("").filter((p) => !elements.slice(1).includes(p));
  } else {
    elements = elements[0] === '[' && elements[elements.length - 1] === ']'
      ? elements.slice(1, -1)
      : elements;
  }
  ...
  for (let i = 0; i < permutationLength; i++) {
    permutations = permutations.flatMap((p) =>
      elements.filter((e) => !p.includes(e)).map((e) => [...p, e])
    );
  }
...
function countPieces(string) {
  let count = 0;
  if (string.includes("^")) {
    string = "IOSZJLT".split("").filter((p) => !string.slice(string.indexOf("^")).includes(p));
  }
...
function queueSplit(string) {
  ...
  for (const char of string) {
    ...
    if (!bagMode && "IOSZJLT".includes(upperChar)) {
...
function sfinderAllPermutations(inputStr) {
  const inputStrSplit = queueSplit(inputStr);
  const inputs = inputStrSplit.split(",");

  const permutations = [];
  for (let inputStr of inputs) {
    inputStr = inputStr
      .replace("^", "[^]")
      .replace("!", "p" + countPieces(inputStr));
    if (!inputStr.includes("p")) {
      inputStr += "p1";
    }
    const [bagWithBrackets, permutationLength] = inputStr.split("p");
    const bag = bagWithBrackets
      .replace("*", "[IOSZJLT]")
      .replace("[", "")
      .replace("]", "")
      .toUpperCase();

    permutations.push(generatePermutations(bag, parseInt(permutationLength, 10)));
  }

  return combineLists(permutations);
}

function holdReorders(queue) {
  if (queue.length <= 1) {
    return new Set(queue); // base case
  }

  const result = new Set();

  const a = holdReorders(queue.slice(1)); // use first piece, work on the 2nd-rest
  for (const part of a) {
    result.add(queue[0] + part);
  }

  const b = holdReorders(queue[0] + queue.slice(2)); // use second piece, work on 1st + 3rd-rest
  for (const part of b) {
    result.add(queue[1] + part);
  }
...
function holdsFinderQueues(queue) {
  const properHold = new Set();
  for (const i of sfinderAllPermutations(queue)) {
    for (const j of holdReorders(i)) {
      properHold.add(j);
    }
  }

  return Array.from(properHold);
}
...
let queues = sfinderAllPermutations(queuesinput);
