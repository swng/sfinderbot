...
function noHoles(fumen) {
    ...
    let field = pages[pages.length - 1].field.copy();
...
function clearLines(fumen) {
    let pages = decoder.decode(fumen);
    let the_field = pages[0].field.copy();
    the_field.clearLine();
    pages.push({ field: the_field });
    return encoder.encode(pages);
}
...
async function main() {
    ...
    let first_piece_results = await generate(initialField, before_piece);
    
    let second_piece_results_all = new Set();
    for (const field_a_fumen of first_piece_results) {
        let second_piece_results = await generate(decoder.decode(field_a_fumen)[0].field, after_piece);
        second_piece_results.forEach(item => second_piece_results_all.add(item));
    }
    ...
    pieceResults.unshift(second_piece_results_all);
    ...
    if (!skims_allowed) {
        ...
        results = Array.from(results).map(clearLines); // each fumen will now have 2 frames, before and after clearLines
        // console.log("douhfaewouhae", merge_fumen(results));
    }

    if (!holes_allowed) {
        results = Array.from(results).filter(noHoles); // set to process on the last frame of the fumen (after the clearline)
    ...
    if (do_sfinder_check) {
        ...
        let glued_candidates = glue(results.map(result => {
            let pages = decoder.decode(result);
            return encoder.encode([pages[0]]);
        }));
...
args.forEach((arg) => {
    ...
    } else if (arg.startsWith('before_piece=')) {
        before_piece = arg.slice(13);
    } else if (arg.startsWith('after_piece=')) {
        after_piece = arg.slice(12);
